    ...
    async function displaydata(){
        ...
        if(income.length>0){
            html += `<h3 style="color: #16a34a; margin-bottom: 15px;">Income Transactions</h3>`;
            html += createTable(income,"income");
        }

        
        if(expense.length>0){
            html += `<h3 style="color: #dc2626; margin-bottom: 15px;">Expense Transactions</h3>`;
            html += createTable(expense,"expense");
        }
